...
function viewDetails(matchID) {
  ...
  if (matchContent) {
    ...
    secondCont.className = "px-4 bg-white w-full";

    secondCont.innerHTML = `
            <!--Match, date, lieu-->
        <div class="mt-6">
            <h1 class="my-2 heading text-blue-950 font-semibold text-2xl mb-4">${matchContent.teams.home} vs. ${matchContent.teams.away}  - Quarter Finals Euro 2024</h1>
            <p class="text-gray-500 font-medium text-xs">${matchContent.simpleDate} at ${matchContent.teams.hour}, ${matchContent.location}</p>    
        </div>
        ...
        <div class="my-4">
            <div class="relative inline-block">
                ...
                <button id="dropdown_btn" class="bg-blue-950 text-white relative flex items-center z-10 p-2 text-sm text-gray-600 border border-transparent rounded-3xl">
                    <span class="mx-1" id="num_tickets">Choose your number of tickets →</span> 
                </button>
        ...
        <div class="py-2 border-t grid grid-cols-2">
  ...
  // imgCont.appendChild(imgContainer);
    ...
    for (let j = 0; j < matchContent.categories.length; j++) {
      ...
      thirdCont.innerHTML = `
                  <div class="grid grid-cols-8 items-center p-2 border-t">
                     ...
                     <div class="col-span-3 text-right font-medium text-normal">€ ${matchContent.categories[j].price}<div class="text-gray-400 font-normal text-xs text-right">Price per ticket</div>
</div>
                  </div>
       `;
            ...
            containerPop.innerHTML = `
                      ...
                      <div class="grid grid-cols-6 ">
                        <div class="col-span-5">
                          ...
                          <div class="text-base font-medium">${valueCase.getAttribute('data-index')} × € ${
              matchContent.categories[index].price
            } / <span class="inline-block text-green-400 font-bold">Total price : €  ${
              valueCase.getAttribute('data-index') * matchContent.categories[index].price
            } </span></div>  
                        </div>
