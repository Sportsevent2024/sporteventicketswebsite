...
//const feeTickets = ((price * 0.3).toFixed(0)).toLocaleString('en-GB', { style: 'currency', currency: 'EUR', minimumFractionDigits: 0, maximumFractionDigits: 2 });
...
secGrid.innerHTML = `
     <div>
         ...
         <div class="mb-6 payment_form">
          <div class="font-semibold mb-4 pb-2 border-b">Payment Type</div>
            ...
            <div class="hidden peer-checked/draft:block text-sm">
              <div class="mt-4">              With this option, you will facilitate transactions between our suppliers for quick satisfaction and be spared fee on tickets.              <div>
                ...
                <div class="grid grid-cols-2 items-center my-4">
                  ...
                  <h1 class="text-sm text-right text-gray-400">${ticketCount} × <span class="text-black">€${Number(price).toLocaleString('en-GB')}</span></h1>
                </div>
                ...
                <div class="grid grid-cols-2 items-center mb-4 font-medium text-xl">
                  <h1>Total Price</h1>
                  <h1 class="text-right">€${Number(total).toLocaleString('en-GB')}</h1>
                </div>
              </div>
            </div>
            ...
            <div class="hidden peer-checked/published:block text-sm">
              ...
              <div class="grid grid-cols-2 items-center my-4">
                ...
                <div class="grid grid-cols-2 items-center mb-4">
                  <h1 class="text-sm">Fee</h1>
                  <h1 class="text-sm text-right text-gray-400">${ticketCount} × <span class="text-black">€${Number(feeTickets).toLocaleString('en-GB')}</span></h1>
                </div>
                <div class="grid grid-cols-2 items-center mb-4 font-medium text-xl">
                  <h1>Total Price</h1>
                  <h1 class="text-right">€${Number(totalFee).toLocaleString('en-GB')}</h1>
                </div>
              </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () =>{
     const paymentBtn = document.querySelector('.paypal_btn');
            ...
            let defaultValue = defaultChecked ? Number(defaultChecked.getAttribute('data--id')) : null;
              ...
              event.preventDefault();
                if (event.target.name === 'status') {
                   // console.log(event.target.getAttribute('data--id'));
                    paymentBtn.setAttribute('href', 'https://paypal.me/sportseventst/'+ Number(event.target.getAttribute('data--id')));
